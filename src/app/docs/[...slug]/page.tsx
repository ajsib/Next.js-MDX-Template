...
export default async function Page({
  ...
  const slugArray = Array.isArray(p.slug) ? p.slug : p.slug ? [p.slug] : []
  const slugPath = slugArray.join("/")
  const breadcrumbs = getBreadcrumbs(slugArray)

  const fileComponent = getMdxBySlug(slugPath)
  const indexExists = hasIndexMdx(slugPath)

  // 1) file at /docs/a/b -> render file
  if (fileComponent && !indexExists) {
    return (
  ...
  // 2) folder with index at /docs/a -> render a/index
  if (indexExists) {
    const indexComponent = getMdxBySlug(`${slugPath}/index`) || fileComponent
    if (!indexComponent) notFound()
    return (
      <MdxSlugLayout navTree={navTree} breadcrumbs={breadcrumbs}>
  ...
  // 3) folder without index -> directory listing
  return (
    <MdxSlugLayout navTree={navTree} breadcrumbs={breadcrumbs}>
